...
Pinia 是 Vue.js 官方推荐的状态管理库，是 Vuex 的后继者。Pinia 提供了更简单、更类型安全、更轻量的 API，用于在 Vue 应用中集中管理组件之间的共享状态。
...
在 Vue 中，常见的 HTTP 请求方式主要有以下几种，通常会搭配第三方库如 `axios` 或浏览器原生的 `fetch` 来发起请求：
...
fetch('/api/users', {
  ...
  body: JSON.stringify({ name: 'Rui' })
})
```

> ⚠️ `fetch` 不会自动抛出网络错误，也不会自动处理 JSON 序列化失败、状态码非 2xx 错误。
